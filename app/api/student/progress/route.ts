...
export async function PATCH(request: Request) {
  ...
  const currentStage =
    body.action === "previous"
      ? Math.max(1, current.currentStage - 1)
      : body.action === "advance"
        ? Math.min(current.totalStages, current.currentStage + 1)
  ...
  return NextResponse.json(
    await prisma.studentProgress.update({
      ...
      data: {
        currentStage,
        hintsUsed: body.action === "hint" ? current.hintsUsed + 1 : undefined,
        peerReviewRequested: body.action === "peer-review" ? true : undefined,
      },
    }),
  );
}
