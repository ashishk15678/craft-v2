...
import {
  OrgAwareRoleGateway,
  OrgDashboardSection,
} from "@/components/org-dashboard-sections";
...
const TRACKS = [
  {
    ...
    tagline: "Vector DB · RAG Engine · Agent Framework",
    color: "text-violet-400 bg-violet-500/10 border-violet-500/30",
  },
  {
    icon: "◫",
    name: "Web & Frameworks",
    tagline: "Virtual DOM · Next.js Router · WS Server",
    color: "text-sky-400 bg-sky-500/10 border-sky-500/30",
  },
  {
    icon: "▣",
    name: "DevOps & Infra",
    tagline: "Mini Docker · CI Runner · Load Balancer",
    color: "text-amber-400 bg-amber-500/10 border-amber-500/30",
  },
  {
    icon: "⌘",
    name: "Fintech & Security",
    tagline: "OAuth2 · Payment Queue · Blockchain",
...
export default async function DashboardPage() {
  ...
  const rps = benchmark
    ? `${(benchmark.requestsPerSecond / 1000).toFixed(0)}k`
    : "—";
    ...
    <div className="w-full space-y-4 pb-10">
        ...
        <div className="p-5 pb-4">
          ...
          <div className="mt-2 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <h1 className="text-2xl font-black uppercase tracking-tight black-ops-one-regular truncate">
                {activeChallenge}
              </h1>
              <p className="mt-1 text-xs text-zinc-400">
                {language} · {activeTrack} · Stage {currentStage}/{totalStages}{" "}
                — <span className="text-indigo-400">{stageName}</span>
              </p>
            </div>
          ...
          <div
            ...
            aria-label={`Stage ${currentStage} of ${totalStages}`}
          >
        ...
        <div className="grid grid-cols-4 border-t border-border divide-x divide-border">
          {[
            ...
            <div key={label} className="px-4 py-3">
              <p className="font-mono text-base font-bold text-text truncate">
                {value}
              </p>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
        ...
        <section className="rounded-xl border border-border bg-card p-5">
          <p className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-indigo-400 mb-3">
            Tools
          </p>
            ...
            {(
              [
                [
                  ...
                  "Paste your error, get a targeted hint.",
                ],
                [
                  "/dashboard/learn",
                  "?",
                  "Stage Hints",
                  "Architecture nudges — no spoilers.",
                ],
                [
                  "/dashboard/portfolio",
                  "◌",
                  "Portfolio Proof",
                  "Verified project cards + GitHub badges.",
                ],
                [
                  "/dashboard/community",
                  "↗",
                  "Community",
                  "Discuss, review, and learn with others.",
                ],
              ] as const
              ...
              >
                ...
                <span className="ml-auto font-mono text-[10px] text-zinc-600 group-hover:text-indigo-400">
                  →
                </span>
              </Link>
            ))}
          ...
          {benchmark && (
            <div className="mt-4 pt-4 border-t border-border">
              <p className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-indigo-400 mb-2">
                Latest benchmark
              </p>
        ...
        <section className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {TRACKS.map((t) => (
              ...
              >
                <span
                  className={`inline-flex h-7 w-7 items-center justify-center rounded-md border font-mono text-sm ${t.color}`}
                >
                  {t.icon}
                </span>
