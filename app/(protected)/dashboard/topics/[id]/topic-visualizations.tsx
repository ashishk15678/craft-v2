    ...
    <div className="space-y-3">
      {steps.map((step: string, i: number) => (
        <div key={i} className="flex items-start gap-3">
          ...
          <div className="flex-1 rounded-lg bg-accent px-4 py-2.5 text-sm">
            {step}
          </div>
          {i < steps.length - 1 && (
            <div
              className="absolute left-3.5 mt-7 h-3 w-px bg-border"
              aria-hidden
            />
          )}
        </div>
        ...
        <thead>
          <tr className="border-b border-border bg-accent">
            {d.headers.map((h, i) => (
              <th
                key={i}
                className="px-4 py-2.5 text-left text-xs font-bold uppercase text-muted-foreground"
              >
                {h}
              </th>
            ))}
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4">
        <p className="mb-3 text-xs font-bold uppercase text-emerald-400">
          Pros
        </p>
...
function TimelineViz({ data }: { data: unknown }) {
  const events =
    (data as { events?: { label: string; description: string }[] })?.events ??
    [];
  return (
    <div className="relative pl-8 space-y-4">
      ...
      {events.map((ev, i) => (
        <div key={i} className="relative">
          <div
            className="absolute -left-[1.25rem] top-1 h-3 w-3 rounded-full border-2 border-indigo-500 bg-background"
            aria-hidden
          />
